...
if (mysqli_connect_errno()){
    ...
    if(isset($_GET['xxCode'])){
        ...
        $invMonth = $_GET['invMonth'];
        $invYear = $_GET['invYear'];
        $merNameSQL = "select merchantName, cod, vat from tbl_merchant_info where merchantCode = '$merchantCode'";
        ...
        // Accumulate existing invoice for the mentioned period
        
        $invoiceSummarySQL = "SELECT destination, productSizeWeight, deliveryOption, sum(totalOrder) as TotalOrder, sum(cash) as cash, sum(Ret) as 'Return', sum(partial) as partial, sum(deliveryCharge) as deliveryCharge, sum(cashCollection) as cashCollection, sum(CashCoD) as CashCoD  FROM `tbl_invoice_details` WHERE invNum in (select invNum from tbl_invoice where merchantCode = '$merchantCode' and inv_date between '$invStartDate' and '$invEndDate' ) group by destination, productSizeWeight, deliveryOption";
        $invoiceSummaryResult = mysqli_query($conn, $invoiceSummarySQL);
        
        //Itemized bill
        
        $invoiceDetailSQL = "select orderid, merOrderRef, productSizeWeight, deliveryOption, packagePrice, CashAmt, tbl_district_info.districtName, charge, case when Cash='Y' then 'Success' when Ret ='Y' then 'Return' when partial = 'Y' then 'Partial' end as deliveryStatus, case when Cash = 'Y' then cashComment when Ret = 'Y' then CONCAT_WS('',retReason ,' ',retRem ) when partial = 'Y' then partialReason end as comment  from tbl_order_details left join tbl_district_info on tbl_order_details.customerDistrict = tbl_district_info.districtId where invNum in (select invNum from tbl_invoice where merchantCode = '$merchantCode' and inv_date between '$invStartDate' and '$invEndDate' ) order by orderDate";
        $invoiceDetailResult = mysqli_query($conn, $invoiceDetailSQL);  
        

        //User Name
        $empNamesql = "select * from tbl_employee_info where empCode = '$user_code'";
...
//    require('fpdf/fpdf.php');
    ...
    if (mysqli_connect_errno()){
        ...
        $pdf->SetXY(150, 40);
        $pdf ->MultiCell(50,4, date('d F, Y', strtotime('+6 hour')), 'LTRB', 'C', 0);
        $pdf->SetXY(100, 44);
        $pdf ->MultiCell(50,4,"Statement Period", '', 'R', 0);
        $pdf->SetXY(150, 44);
        $pdf ->MultiCell(50,4, date("F Y", strtotime($invStartDate)) , 'LTRB', 'C', 0);
        $pdf->SetXY(100, 48);
        $pdf ->MultiCell(50,4,"Statement Prepared By", '', 'R', 0);
        $pdf->SetXY(150, 48);
        $pdf ->MultiCell(50,4, $empNameRow['empName'], 'LTRB', 'C', 0);
        //End of Header Part
        $localTotOrder = 0;
        $localTotCash = 0;
        $localTotReturn = 0;
        ...
        foreach ($invoiceSummaryResult as $invoiceItemRow) {
            //Local Part
            if ($invoiceItemRow['destination']=='local'){
                ...
                $localTotReturn = $localTotReturn + $invoiceItemRow['Return'];
                ...
                $localTotCashCoD = $localTotCashCoD + ($invoiceItemRow['cashCollection'] * ($merchantrow['cod']/100));        
            }
            ...
            if ($invoiceItemRow['destination']=='local' and $invoiceItemRow['productSizeWeight'] == 'standard'  and $invoiceItemRow['deliveryOption'] == 'regular') {
                ...
                $SRReturn = $invoiceItemRow['Return'];
                ...
                $SRCashCoD = ($SRcashCollection * ($merchantrow['cod']/100));
            }
            if ($invoiceItemRow['destination']=='local' and $invoiceItemRow['productSizeWeight'] == 'standard'  and $invoiceItemRow['deliveryOption'] == 'express') {
                ...
                $SEReturn = $invoiceItemRow['Return'];
                ...
                $SECashCoD = ($SEcashCollection * ($merchantrow['cod']/100));
            }
            if ($invoiceItemRow['destination']=='local' and $invoiceItemRow['productSizeWeight'] == 'large'  and $invoiceItemRow['deliveryOption'] == 'regular') {
                ...
                $LRReturn = $invoiceItemRow['Return'];
                ...
                $LRCashCoD = ($LRcashCollection * ($merchantrow['cod']/100));
            }
            if ($invoiceItemRow['destination']=='local' and $invoiceItemRow['productSizeWeight'] == 'large'  and $invoiceItemRow['deliveryOption'] == 'express') {
                ...
                $LEReturn = $invoiceItemRow['Return'];
                ...
                $LECashCoD = ($LEcashCollection * ($merchantrow['cod']/100));
            }
            if ($invoiceItemRow['destination']=='local' and $invoiceItemRow['productSizeWeight'] == 'special'  and $invoiceItemRow['deliveryOption'] == 'regular') {
                ...
                $SPRReturn = $invoiceItemRow['Return'];
                ...
                $SPRCashCoD = ($SPRcashCollection * ($merchantrow['cod']/100));
            }
            if ($invoiceItemRow['destination']=='local' and $invoiceItemRow['productSizeWeight'] == 'special'  and $invoiceItemRow['deliveryOption'] == 'express') {
                ...
                $SPEReturn = $invoiceItemRow['Return'];
                ...
                $SPECashCoD = ($SPEcashCollection * ($merchantrow['cod']/100));
            }
            if ($invoiceItemRow['destination']=='local' and $invoiceItemRow['productSizeWeight'] == 'specialplus'  and $invoiceItemRow['deliveryOption'] == 'regular') {
                ...
                $SPPRReturn = $invoiceItemRow['Return'];
                ...
                $SPPRCashCoD = ($SPPRcashCollection * ($merchantrow['cod']/100));
            }
            if ($invoiceItemRow['destination']=='local' and $invoiceItemRow['productSizeWeight'] == 'specialplus'  and $invoiceItemRow['deliveryOption'] == 'express') {
                ...
                $SPPEReturn = $invoiceItemRow['Return'];
                ...
                $SPPECashCoD = ($SPPEcashCollection * ($merchantrow['cod']/100));
            }
            //Inter District Part
            if ($invoiceItemRow['destination']=='interDistrict'){
                ...
                $interTotReturn = $interTotReturn + $invoiceItemRow['Return'];
                ...
                $interTotCashCoD = $interTotCashCoD + ($invoiceItemRow['cashCollection'] * ($merchantrow['cod']/100));        
            }
            if ($invoiceItemRow['destination']=='interDistrict' and $invoiceItemRow['productSizeWeight'] == 'standard'  and $invoiceItemRow['deliveryOption'] == 'regular') {
                ...
                $ISRReturn = $invoiceItemRow['Return'];
                ...
                $ISRCashCoD = ($ISRcashCollection * ($merchantrow['cod']/100));
            }
            if ($invoiceItemRow['destination']=='interDistrict' and $invoiceItemRow['productSizeWeight'] == 'standard'  and $invoiceItemRow['deliveryOption'] == 'express') {
                ...
                $ISEReturn = $invoiceItemRow['Return'];
                ...
                $ISECashCoD = ($ISEcashCollection * ($merchantrow['cod']/100));
            }
            if ($invoiceItemRow['destination']=='interDistrict' and $invoiceItemRow['productSizeWeight'] == 'large'  and $invoiceItemRow['deliveryOption'] == 'regular') {
                ...
                $ILRReturn = $invoiceItemRow['Return'];
                ...
                $ILRCashCoD = ($ILRcashCollection * ($merchantrow['cod']/100));
            }
            if ($invoiceItemRow['destination']=='interDistrict' and $invoiceItemRow['productSizeWeight'] == 'large'  and $invoiceItemRow['deliveryOption'] == 'express') {
                ...
                $ILEReturn = $invoiceItemRow['Return'];
                ...
                $ILECashCoD = ($ILEcashCollection * ($merchantrow['cod']/100));
            }
            if ($invoiceItemRow['destination']=='interDistrict' and $invoiceItemRow['productSizeWeight'] == 'special'  and $invoiceItemRow['deliveryOption'] == 'regular') {
                ...
                $ISPRReturn = $invoiceItemRow['Return'];
                ...
                $ISPRCashCoD = ($ISPRcashCollection * ($merchantrow['cod']/100));
            }
            if ($invoiceItemRow['destination']=='interDistrict' and $invoiceItemRow['productSizeWeight'] == 'special'  and $invoiceItemRow['deliveryOption'] == 'express') {
                ...
                $ISPEReturn = $invoiceItemRow['Return'];
                ...
                $ISPECashCoD = ($ISPEcashCollection * ($merchantrow['cod']/100));
            }
            if ($invoiceItemRow['destination']=='interDistrict' and $invoiceItemRow['productSizeWeight'] == 'specialplus'  and $invoiceItemRow['deliveryOption'] == 'regular') {
                ...
                $ISPPRReturn = $invoiceItemRow['Return'];
                ...
                $ISPPRCashCoD = ($ISPPRcashCollection * ($merchantrow['cod']/100));
            }
            if ($invoiceItemRow['destination']=='interDistrict' and $invoiceItemRow['productSizeWeight'] == 'specialplus'  and $invoiceItemRow['deliveryOption'] == 'express') {
                ...
                $ISPPEReturn = $invoiceItemRow['Return'];
                ...
                $ISPPECashCoD = ($ISPPEcashCollection * ($merchantrow['cod']/100));
            }
        }
        ...
        $pdf->SetXY(105, 204);
        $pdf ->MultiCell(20,4, num_to_format(round($discAdj)), 'LTRB', 'R', 0);
        $pdf->SetFont('Helvetica','B',8);
        $pdf->SetXY(70, 208);
        $pdf ->MultiCell(35,4,"Total Charges Deducted", '', 'R', 0);
        $pdf->SetXY(105, 208);
        $pdf ->MultiCell(20,4, num_to_format(round(($localTotCashCoD + $interTotCashCoD + $localTotDeliveryChg + $interTotDeliveryChg)-$discAdj)), 'LTRB', 'R', 0);
        if($merchantrow['vat'] > 0){
            $pdf->SetFont('Helvetica','',8);
            $pdf->SetXY(70, 212);
            $pdf ->MultiCell(35,4,"VAT ".$merchantrow['vat'].'%', '', 'R', 0);
            $pdf->SetXY(105, 212);
            $pdf ->MultiCell(20,4, num_to_format(round(($localTotCashCoD + $interTotCashCoD + $localTotDeliveryChg + $interTotDeliveryChg)*($merchantrow['vat']/100))), 'LTRB', 'R', 0);
            $pdf->SetFont('Helvetica','B',8);
            $pdf->SetXY(61, 216);
            $pdf ->MultiCell(45,4,"Total Charges Including VAT ", '', 'R', 0);
            $pdf->SetXY(105, 216);
            $pdf ->MultiCell(20,4, num_to_format(round(($localTotCashCoD + $interTotCashCoD + $localTotDeliveryChg + $interTotDeliveryChg)-$discAdj) + round(($localTotCashCoD + $interTotCashCoD + $localTotDeliveryChg + $interTotDeliveryChg)*($merchantrow['vat']/100))), 'LTRB', 'R', 0);                            
        }
        $pdf->SetFont('Helvetica','',8);
        ...
        $pdf ->MultiCell(20,4,"Amount(BDT)", '', 'C', 0);
        $pdf->SetXY(175, 192);
        $pdf ->MultiCell(20,4, num_to_format(abs(0 - $totCharges)), 'LTRB', 'R', 0);
        $pdf->SetXY(130, 196);
        $pdf ->MultiCell(45,4, "(Paperfly charges only)", '', 'R', 0);


        $pdf->SetXY(35, 224);
        $pdf ->MultiCell(140,4, $invComment1, '', 'L', 0);
        $pdf->SetXY(35, 228);
        $pdf ->MultiCell(140,4, $invComment2, '', 'L', 0);
        $pdf->SetXY(35, 232);
        $pdf ->MultiCell(140,4, $invComment3, '', 'L', 0);
        $pdf->SetXY(35, 236);
        $pdf ->MultiCell(140,4, $invComment4, '', 'L', 0);
        $pdf->SetXY(35, 240);
        $pdf ->MultiCell(140,4, $invComment5, '', 'L', 0);
        $pdf->SetXY(35, 244);
        $pdf ->MultiCell(140,4, $invComment6, '', 'L', 0);
        ...
        if (mysqli_num_rows($invoiceDetailResult) > 0) {
            ...
            foreach ($invoiceDetailResult as $invoiceDetailRow){
                if ($lineCount == 60) {
                    ...
                    $pdf ->MultiCell(30,3, substr($invoiceDetailRow['merOrderRef'],0,25), 'LBRT', 'L', 0);
