...
if (mysqli_connect_errno()){
    ...
    if(isset($_GET['xxCode'])){
        ...
        $merNameSQL = "select merchantName, returnCharge from tbl_merchant_info where merchantCode = '$merchantCode'";
        $merNameResult = mysqli_query($conn, $merNameSQL);
        $merchantrow = mysqli_fetch_array($merNameResult);
        $returnCharge = $merchantrow['returnCharge'];

        //invoice period
        $invPeriodSQL = "SELECT statementDate from tbl_merchant_info where merchantCode = '$merchantCode'";
        ...
        //Date for order those are not close withing statement date
        $stmntDate = date('Y-m-d', strtotime('+6 hour'));

        if($returnCharge == 'Y'){
            $invoiceItemSQL = "select destination, productSizeWeight, deliveryOption, count(orderid) as TotalOrder, count(cash) as cash, count(Ret) as 'Return', count(partial) as 'partial', sum(charge) as deliveryCharge, sum(CashAmt) as cashCollection, IF(sum(CashAmt)=0, 0, sum(CashAmt)-sum(charge)) * (cod/100) as CashCoD from tbl_order_details where merchantCode = '$merchantCode' and close = 'Y' and invNum IS NULL and (DATE_FORMAT(closeTime,'%Y-%m-%d') between '$startDate' and '$invEnddate') and orderType in ('Merchant', 'Other_Merchant') group by destination, productSizeWeight, deliveryOption";
            $invoiceItemResult = mysqli_query($conn, $invoiceItemSQL);

            //Itemized orders
            $invoiceDetailSQL = "select orderid, merOrderRef, productSizeWeight, deliveryOption, packagePrice, CashAmt, tbl_district_info.districtName, charge, case when Cash = 'Y' then 'Success' when Ret = 'Y' then 'Return' when partial = 'Y' then 'partial' end as deliveryStatus, case when Cash = 'Y' then cashComment when Ret = 'Y' then CONCAT_WS('',retReason ,' ',retRem ) when partial = 'Y' then partialReason when orderType='smartPick' then smartPickComment end as comment from tbl_order_details left join tbl_district_info on tbl_order_details.customerDistrict = tbl_district_info.districtId where merchantCode = '$merchantCode' and close = 'Y' and invNum IS NULL and DATE_FORMAT(closeTime,'%Y-%m-%d') between '$startDate' and '$invEnddate' and orderType in ('Merchant', 'Other_Merchant') order by orderDate, deliveryStatus desc";
            $invoiceDetailResult = mysqli_query($conn, $invoiceDetailSQL);            
        } else {
            $invoiceItemSQL = "select destination, productSizeWeight, deliveryOption, count(orderid) as TotalOrder, count(cash) as cash, count(Ret) as 'Return', count(partial) as 'partial', sum(IF(Ret = 'Y',0,charge)) as deliveryCharge, sum(CashAmt) as cashCollection, IF(sum(CashAmt)=0, 0, sum(CashAmt)-sum(charge)) * (cod/100) as CashCoD from tbl_order_details where merchantCode = '$merchantCode' and close = 'Y' and invNum IS NULL and (DATE_FORMAT(closeTime,'%Y-%m-%d') between '$startDate' and '$invEnddate') and orderType in ('Merchant', 'Other_Merchant') group by destination, productSizeWeight, deliveryOption";
            $invoiceItemResult = mysqli_query($conn, $invoiceItemSQL);

            //Itemized orders
            $invoiceDetailSQL = "select orderid, merOrderRef, productSizeWeight, deliveryOption, packagePrice, CashAmt, tbl_district_info.districtName, IF(Ret = 'Y', 0, charge) as charge, case when Cash = 'Y' then 'Success' when Ret = 'Y' then 'Return' when partial = 'Y' then 'partial' end as deliveryStatus, case when Cash = 'Y' then cashComment when Ret = 'Y' then CONCAT_WS('',retReason ,' ',retRem ) when partial = 'Y' then partialReason when orderType='smartPick' then smartPickComment end as comment from tbl_order_details left join tbl_district_info on tbl_order_details.customerDistrict = tbl_district_info.districtId where merchantCode = '$merchantCode' and close = 'Y' and invNum IS NULL and DATE_FORMAT(closeTime,'%Y-%m-%d') between '$startDate' and '$invEnddate' and orderType in ('Merchant', 'Other_Merchant') order by orderDate, deliveryStatus desc";
            $invoiceDetailResult = mysqli_query($conn, $invoiceDetailSQL);            
        }
...
//    require('fpdf/fpdf.php');
    ...
    if (mysqli_connect_errno()){
        ...
        $pdf->SetXY(150, 44);
        $pdf ->MultiCell(50,4, date('d F, Y', strtotime($invPeriodRow['statementDate']))." to ".date('d F, Y', strtotime($invEnddate)) , 'LTRB', 'C', 0);
        $pdf->SetXY(100, 48);
        ...
        if (mysqli_num_rows($invoiceDetailResult) > 0) {
            ...
            foreach ($invoiceDetailResult as $invoiceDetailRow){
                if ($lineCount == 60) {
                    ...
                    $pdf ->MultiCell(30,3, substr($invoiceDetailRow['merOrderRef'],0,26), 'LBRT', 'L', 0);
        ...
        $PinvoiceDetailSQL = "select orderid, merOrderRef, productSizeWeight, deliveryOption, packagePrice, CashAmt, tbl_district_info.districtName, charge, case when Cash = 'Y' then 'Success' when Ret = 'Y' then 'Return' when partial ='Y' then 'partial' end as deliveryStatus, case when Cash = 'Y' then cashComment when Ret = 'Y' then CONCAT_WS('',retReason ,' ',retRem ) when partial = 'Y' then partialReason end as comment from tbl_order_details left join tbl_district_info on tbl_order_details.customerDistrict = tbl_district_info.districtId where merchantCode = '$merchantCode' and close IS NULL and invNum IS NULL and orderDate < '$stmntDate' and orderType in ('Merchant', 'Other_Merchant') order by orderDate, deliveryStatus desc";
        ...
        if (mysqli_num_rows($PinvoiceDetailResult) > 0) {
            ...
            foreach ($PinvoiceDetailResult as $PinvoiceDetailRow){
                if ($lineCount == 60) {
                    ...
                    $pdf->SetXY(35, $yPosition);
                    $pdf ->MultiCell(30,3, substr($PinvoiceDetailRow['merOrderRef'],0,26), 'LBRT', 'L', 0);
                    $pdf->SetXY(65, $yPosition);
