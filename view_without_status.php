<?php
    ...
    if(isset($_GET['xxCode'])){
        ...
        $empPointCode = encryptor('decrypt', $zzCode);
        ...
        if($_GET['yyCode'] !=''){
            ...
            $pointOrdersResult = mysqli_query($conn, "SELECT orderid, merOrderRef, tbl_merchant_info.merchantName, IFNULL(tbl_employee_info.empName,'') as dropExecutive, tbl_thana_info.thanaName, tbl_district_info.districtName, IFNULL(Cash,'') as Cash, IFNULL(Ret,'') as Ret, IFNULL(partial,'') as partial, IFNULL(Rea,'') as Rea, IFNULL(onHoldSchedule,'') as onHoldSchedule FROM tbl_order_details left join tbl_employee_info on tbl_order_details.dropPointEmp = tbl_employee_info.empCode left join tbl_merchant_info on tbl_order_details.merchantCode = tbl_merchant_info.merchantCode left join tbl_thana_info on tbl_order_details.customerThana = tbl_thana_info.thanaid left join tbl_district_info on tbl_order_details.customerDistrict = tbl_district_info.districtid WHERE tbl_order_details.merchantCode = '$merchant' and dropPointCode = '$pointCode' and Shtl = 'Y' and Cash is null and Ret is null and partial is null and Rea is null and close is null order by orderDate");
        } else {
            $pointOrdersResult = mysqli_query($conn, "SELECT orderid, merOrderRef, tbl_merchant_info.merchantName, IFNULL(tbl_employee_info.empName,'') as dropExecutive, tbl_thana_info.thanaName, tbl_district_info.districtName, IFNULL(Cash,'') as Cash, IFNULL(Ret,'') as Ret, IFNULL(partial,'') as partial, IFNULL(Rea,'') as Rea, IFNULL(onHoldSchedule,'') as onHoldSchedule FROM tbl_order_details left join tbl_employee_info on tbl_order_details.dropPointEmp = tbl_employee_info.empCode left join tbl_merchant_info on tbl_order_details.merchantCode = tbl_merchant_info.merchantCode left join tbl_thana_info on tbl_order_details.customerThana = tbl_thana_info.thanaid left join tbl_district_info on tbl_order_details.customerDistrict = tbl_district_info.districtid WHERE dropPointCode = '$pointCode' and Shtl = 'Y' and Cash is null and Ret is null and partial is null and Rea is null and close is null order by orderDate");    
        }
