...
if (mysqli_connect_errno()){
    ...
    if(isset($_GET['xxCode'])){
        ...
        $merNameSQL = "select merchantName from tbl_merchant_info where merchantCode = '$merchantCode'";
        ...
        //Itemized orders
        $invoiceDetailSQL = "select orderid, merOrderRef, productSizeWeight, deliveryOption, packagePrice, CashAmt, tbl_district_info.districtName, charge, case when Cash = 'Y' then 'Success' when Ret = 'Y' then 'Return' when partial = 'Y' then 'partial' end as deliveryStatus, case when Cash = 'Y' then CONCAT_WS('',cashComment ,' ',smartPickComment ) when Ret = 'Y' then CONCAT_WS('',retReason ,' ',retRem,' ', smartPickComment ) when partial = 'Y' then CONCAT_WS('',partialReason ,' ',smartPickComment ) when orderType='smartPick' then smartPickComment end as comment from tbl_order_details left join tbl_district_info on tbl_order_details.customerDistrict = tbl_district_info.districtId where merchantCode = '$merchantCode' and close = 'Y' and invNum IS NULL and DATE_FORMAT(closeTime,'%Y-%m-%d') between '$startDate' and '$invEnddate' and orderType ='smartPick' order by orderDate, deliveryStatus desc";
...
//    require('fpdf/fpdf.php');
    ...
    if (mysqli_connect_errno()){
        ...
        $pdf->SetXY(150, 44);
        $pdf ->MultiCell(50,4, date('d F, Y', strtotime($invPeriodRow['smartStatementDate']))." to ".date('d F, Y', strtotime($invEnddate)) , 'LTRB', 'C', 0);
        $pdf->SetXY(100, 48);
        ...
        if (mysqli_num_rows($PinvoiceDetailResult) > 0) {
            ...
            foreach ($PinvoiceDetailResult as $PinvoiceDetailRow){
                if ($lineCount == 60) {
                    ...
                    $pdf->SetXY(35, $yPosition);
                    $pdf ->MultiCell(30,3, substr($PinvoiceDetailRow['merOrderRef'],0,25), 'LBRT', 'L', 0);
                    $pdf->SetXY(65, $yPosition);
