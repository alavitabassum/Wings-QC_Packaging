<?php
    ...
    include('header.php');
    include('config.php');
    $pickAssignSQL = "SELECT tbl_order_details.merchantCode, tbl_merchant_info.merchantName, count(1) as cnt FROM `tbl_order_details` left join tbl_merchant_info on tbl_order_details.merchantCode = tbl_merchant_info.merchantCode WHERE dropPointEmp is null and close is null group by tbl_order_details.merchantCode";
    $pickAssignResult = mysqli_query($conn, $pickAssignSQL);
    $pickEmpSQL = "Select empCode, empName from tbl_employee_info where desigid in (5,6,8,9,10) and tbl_employee_info.isActive = 'Y'";
    $pickEmpResult = mysqli_query($conn, $pickEmpSQL);
?>
        <div style="margin-left: 15px; width: 98%; clear: both">
            <p style="background-color: #16469E; border-radius: 5px; width: 100%; height: 25px; color: #fff; font: 15px 'paperfly roman'">Drop Assignment in Batch</p>
            ...
            <table class="table table-hover" id="pickAssign-table">
                ...
                <tbody>
                    <?php foreach($pickAssignResult as $pickAssignRow){?>
                    <tr>
