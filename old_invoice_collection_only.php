...
//    require('fpdf/fpdf.php');
    ...
    if (mysqli_connect_errno()){
        ...
        $pdf->SetXY(86, 32);
        $pdf ->MultiCell(35,4,"Collection from customer part only (excluding Paperfly charges)", '', 'C', 0);
        ...
        $pdf->SetXY(150, 36);
        $pdf ->MultiCell(50,4, $invoiceHeaderRow['invNum'], 'LTRB', 'C', 0);
        $pdf->SetXY(100, 40);
        $pdf ->MultiCell(50,4,"Statement Date", '', 'R', 0);
        $pdf->SetXY(150, 40);
        $pdf ->MultiCell(50,4, date('d F, Y', strtotime($invoiceHeaderRow['inv_date'])), 'LTRB', 'C', 0);
        $pdf->SetXY(100, 44);
        $pdf ->MultiCell(50,4,"Statement Period", '', 'R', 0);
        $pdf->SetXY(150, 44);
        $pdf ->MultiCell(50,4, date('d F, Y', strtotime($invoiceHeaderRow['minDate']))." to ".date('d F, Y', strtotime($invoiceHeaderRow['maxDate'])) , 'LTRB', 'C', 0);
        $pdf->SetXY(100, 48);
        ...
        $pdf->SetXY(91.8, 76);
        $pdf ->MultiCell(13,4, '0', 'LTRB', 'R', true);
        $pdf->SetXY(109, 76);
        ...
        $pdf->SetXY(154, 76);
        $pdf ->MultiCell(15,4, '0', 'LTRB', 'R', true);
        $pdf->SetXY(169, 76);
        $pdf ->MultiCell(15,4,"", 'LTRB', 'R', true);
        $pdf->SetXY(185, 76);
        $pdf ->MultiCell(15,4, '0', 'LTRB', 'R', true);
        ...
        $totCharges = 0; //round(($localTotCashCoD + $interTotCashCoD + $localTotDeliveryChg + $interTotDeliveryChg)-$invoiceHeaderRow['Discount']);
        if (($totCollections - $totCharges) > 0){
            $pdf->SetXY(140, 192);
            $pdf ->MultiCell(35,4, "Payable to Merchant", '', 'R', 0);
            $pdf->SetFont('Helvetica','',6);
            $pdf->SetXY(130, 196);
            $pdf ->MultiCell(45,4, "(excluding Paperfly charges)", '', 'R', 0);
            $pdf->SetFont('Helvetica','',8);
        } else {
            $pdf->SetXY(130, 192);
            $pdf ->MultiCell(45,4, "Receivable from Merchant", '', 'R', 0);
            $pdf->SetXY(130, 196);
            $pdf ->MultiCell(45,4, "(excluding Paperfly charges)", '', 'R', 0);
            $pdf->SetFont('Helvetica','',8);
        }
        ...
        if (mysqli_num_rows($invoiceDetailResult) > 0) {
            ...
            $pdf ->MultiCell(50,4,"Statement Date", '', 'R', 0);
            $pdf->SetXY(150, 40);
            $pdf ->MultiCell(50,4, date('d F, Y', strtotime($merchantrow['statementDate'])), 'LTRB', 'C', 0);
            $pdf->SetXY(100, 44);
            $pdf ->MultiCell(50,4,"Statement Period", '', 'R', 0);
            $pdf->SetXY(150, 44);
            $pdf ->MultiCell(50,4, date('d F, Y', strtotime($invoiceHeaderRow['minDate']))." to ".date('d F, Y', strtotime($invoiceHeaderRow['maxDate'])) , 'LTRB', 'C', 0);
            ...
            $orderCount = 1;
            $itemized_total_collection = 0;
            $itemized_total_price = 0; 
            $no_of_order = mysqli_num_rows($invoiceDetailResult);
            foreach ($invoiceDetailResult as $invoiceDetailRow){
                if ($lineCount == 60) {
                    ...
                    $pdf->SetFont('Helvetica','',6);
                    $itemized_total_collection =  $itemized_total_collection + $invoiceDetailRow['CashAmt'];
                    $itemized_total_price = $itemized_total_price + $invoiceDetailRow['packagePrice'];
                } else {
                    ...
                    $itemized_total_collection =  $itemized_total_collection + $invoiceDetailRow['CashAmt'];
                    $itemized_total_price = $itemized_total_price + $invoiceDetailRow['packagePrice'];
                    if ($orderCount == $no_of_order){
                        $pdf->SetXY(65, $yPosition+3);
                        $pdf ->MultiCell(37,3, "Total     ", 'LRBT', 'C', 0);
                        $pdf->SetXY(90, $yPosition+3);
                        $pdf ->MultiCell(12,3, num_to_format(round($itemized_total_price)), 'LBRT', 'R', 0);                                              
                        $pdf->SetXY(102, $yPosition+3);
                        $pdf ->MultiCell(14,3, num_to_format(round($itemized_total_collection)), 'BRT', 'R', 0);                     
                    }                                                                                                                                       
                }
                $yPosition = $yPosition +3;
                $lineCount ++;
                ...
                if ($lineCount == 60){
                    $pdf->SetXY(65, $yPosition);
                    $pdf ->MultiCell(37,3, "Total     ", 'LBRT', 'C', 0); 
                    $pdf->SetXY(90, $yPosition);
                    $pdf ->MultiCell(12,3, num_to_format(round($itemized_total_price)), 'LBRT', 'R', 0); 
                    $pdf->SetXY(102, $yPosition);
                    $pdf ->MultiCell(14,3, num_to_format(round($itemized_total_collection)), 'BRT', 'R', 0);                      
