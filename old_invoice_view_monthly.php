...
if (mysqli_connect_errno()){
    ...
    if(isset($_GET['xxCode'])){
        ...
        $invoiceHeaderSQL = "select * from tbl_invoice where monthInvNum ='$invoiceNumber'";
        ...
        $merNameSQL = "select merchantName, cod, vat, statementDate from tbl_merchant_info where merchantCode = '$merchantCode'";
        $merNameResult = mysqli_query($conn, $merNameSQL);
        $merchantrow = mysqli_fetch_array($merNameResult);

        $invoiceItemSQL = "SELECT destination, productSizeWeight, deliveryOption, sum(totalOrder) as TotalOrder, sum(cash) as cash, sum(Ret) as 'Ret', sum(partial) as partial, sum(deliveryCharge) as deliveryCharge, sum(cashCollection) as cashCollection, sum(CashCoD) as CashCoD  FROM `tbl_invoice_details` WHERE invNum in ( select invNum from tbl_invoice where monthInvNum = '$invoiceNumber') group by destination, productSizeWeight, deliveryOption";
        ...
        //User Name
        $usr = $invoiceHeaderRow['monthInvUser'];
...
//    require('fpdf/fpdf.php');
    ...
    if (mysqli_connect_errno()){
        ...
        $pdf->SetXY(150, 36);
        $pdf ->MultiCell(51,4, $invoiceHeaderRow['monthInvNum'], 'LTRB', 'C', 0);
        $pdf->SetXY(100, 40);
        $pdf ->MultiCell(50,4,"Statement Date", '', 'R', 0);
        $pdf->SetXY(150, 40);
        $pdf ->MultiCell(51,4, date('d F, Y', strtotime($invoiceHeaderRow['statementDate'])), 'LTRB', 'C', 0);
        $pdf->SetXY(100, 44);
        $pdf ->MultiCell(50,4,"Statement Period", '', 'R', 0);
        $pdf->SetXY(150, 44);
        $pdf ->MultiCell(51,4, date('F, Y', strtotime($invoiceHeaderRow['invMonth'])), 'LTRB', 'C', 0);
        $pdf->SetXY(100, 48);
        ...
        $pdf->SetXY(105, 204);
        $pdf ->MultiCell(20,4, num_to_format(round($invoiceHeaderRow['monthDiscount'])), 'LTRB', 'R', 0);
        $pdf->SetFont('Helvetica','B',8);
        $pdf->SetXY(70, 208);
        $pdf ->MultiCell(35,4,"Total Charges Deducted", '', 'R', 0);
        $pdf->SetXY(105, 208);
        $pdf ->MultiCell(20,4, num_to_format(round(($localTotCashCoD + $interTotCashCoD + $localTotDeliveryChg + $interTotDeliveryChg)-$invoiceHeaderRow['monthDiscount'])), 'LTRB', 'R', 0);
        ...
        $totCharges = round(($localTotCashCoD + $interTotCashCoD + $localTotDeliveryChg + $interTotDeliveryChg)-$invoiceHeaderRow['monthDiscount']);
        if ((0 - $totCharges) > 0){
            $pdf->SetXY(140, 192);
        ...
        $pdf ->MultiCell(20,4,"Amount(BDT)", '', 'C', 0);
        $pdf->SetXY(175, 192);
        $pdf ->MultiCell(20,4, num_to_format($totCharges), 'LTRB', 'R', 0);

        $pdf->SetXY(35, 224);
        $pdf ->MultiCell(140,4, $invoiceHeaderRow['monthMessage'], '', 'L', 0);
